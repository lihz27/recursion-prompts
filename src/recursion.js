...
var gcd = function(x, y) { //24, 88
  ...
  return gcd(smallest, largest % smallest); // gcd (24, 16) // gcd (16, 8)
};
...
var binarySearch = function(array, target, min, max) {
  ...
  var min = min === undefined ? 0 : min; //0 //0 // 0
  let halfway = Math.floor((array.length/2));   //7 ->3 -> 1  //16/2 = 8  // 7/2 = 3.5 -> 3 // 3/2 = 1 // 0

  if (array[halfway] === target) { //array[8] === 15  // array[3] === 15, 12 === 15 // array[1] === 15, 14 === 15 // 15 === 15
    return min + halfway; // 15 + 0
  } else if (array[0] !== target && array.length === 1) {
    return null;
  } else if (target < array[halfway]) { //15 < 8 // 15 < 12 // 15 < 14
    return binarySearch(array.slice(0, halfway), target);
  } else {
    min += halfway + 1; // min = 9 // min = 0 + halfway + 1 = 4 // min = 0+1 +1 = 2 
    if (binarySearch(array.slice(min), target) === null) {
      return null;
    } else {  // min is always the same as halfway + 1, could potentially replace halfway+1 with min 
      return min + binarySearch(array.slice(min), target);  // 9 + binarySearch(array.slice(9), 15) // 4 + binary(array.slice(4), 15) // 2 + binarySearch(array(2), 15)
    }
  }
};
